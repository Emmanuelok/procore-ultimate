...
| 12 | ~~**No IP allowlisting, session timeout policy, or password policy configuration**~~ — **CLOSED, with one qualifier stated rather than buried.** A per-tenant policy covers all three plus lockout thresholds and password history, and the strictest policy across a user's companies wins (§1.9). The allowlist is applied twice — at sign-in, and in `requireCompany` on every company-scoped request — so a session opened inside an approved network stops reading that tenant's data from outside it. The **idle** timeout is the qualifier: the logic is `touchSession` (`modules/account/sessions.ts`), and it fires platform-wide only if `plugins/auth.ts` calls it. Where it does not, `/account/*` is the only place it bites and everyone else is bounded by the absolute lifetime instead. Grep `touchSession` in `plugins/auth.ts` to see which of the two you are running | Vol I #23–25 |
