/**
 * Evidence multi-select for social value delivery records. A delivered
 * commitment that cannot be evidenced is an assertion, and on UK public work
 * assertions get audited (#539) — so the delivery form asks for the evidence
 * ids at the moment of recording rather than later.
 */
import { useEffect, useState } from "react";
import { api } from "../../lib/api";
import { Badge, Spinner } from "../../ui";
import { formatDate, humanize } from "../format";
import type { EvidencePickRow, ListResponse } from "./esgShared";
...
export default function EvidenceSelect({
  ...
  const [items, setItems] = useState<EvidencePickRow[] | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await api.get<ListResponse<EvidencePickRow>>(
  ...
  if (items.length === 0) {
    return (
      <div className="rounded-md border border-dashed border-ink-200 px-3 py-4 text-center text-xs text-ink-400">
        No evidence captured in this project yet. Ingest the apprenticeship record, the local-spend
        report or the volunteering log in the Assurance workspace first — a delivery with no
        evidence behind it will not survive a tender-compliance audit.
      </div>
    );
  }
