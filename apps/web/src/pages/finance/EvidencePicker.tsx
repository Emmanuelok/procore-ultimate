/**
 * Evidence multi-select for the finance workspace: facility conditions are
 * satisfied WITH evidence (#730-731) and disbursement requests assemble
 * expenditure evidence (#732) — this picker is how those ids get chosen.
 */
import { useEffect, useState } from "react";
import { api } from "../../lib/api";
import { Badge, Spinner } from "../../ui";
import { formatDate, humanize } from "../format";
import type { EvidenceRow, ListResponse } from "./financeShared";
...
export default function EvidencePicker({
  ...
  /** shown when the selection is mandatory (condition satisfaction) */
  requiredNote?: string;
}) {
  ...
  if (items.length === 0) {
    return (
      <div className="rounded-md border border-dashed border-ink-200 px-3 py-4 text-center text-xs text-ink-400">
        No evidence captured in this project yet. Ingest evidence in the Assurance workspace
        first — {requiredNote ?? "evidence records substantiate finance actions"}.
      </div>
    );
  }
    ...
    <div>
      ...
      <div className="mt-1 flex items-center justify-between text-xs text-ink-500">
        <span>
          ...
          {requiredNote && selected.length === 0 ? (
            <span className="ml-1.5 font-medium text-amber-700">— {requiredNote}</span>
          ) : null}
        </span>
        {selected.length > 0 ? (
          <button
