...
export interface EvidenceLike {
  ...
  submittedBy: string;
  /** whatever the inlet that wrote the row recorded about how it arrived */
  provenance?: unknown;
}
...
/**
 * Below this prior an evidence kind is an ATTESTATION: content a person
 * composes — a photograph they framed, a video they shot, a document they
 * wrote, "other". At or above it the kind is a RECORD: content that exists
 * upstream of whoever uploads it, generated by an instrument (turnstile,
 * biometric reader, telematics unit, sensor, scanner), an institution (bank,
 * corporate registry, a supplier's delivery note) or a professional measure
 * (survey, inspection). `INDEPENDENCE_BY_KIND` has a natural gap at this
 * value — delivery_note 0.6 above it, video 0.45 below — so the threshold is
 * read off the table rather than chosen as a list of kinds.
 */
export const ATTESTATION_INDEPENDENCE_CEILING = 0.5;

/**
 * Is this row the SUBMITTER'S OWN ATTESTATION, or a record they merely
 * conveyed?
 *
 * The separation rule (Vol III §4) is about where evidence COMES FROM, not
 * who pressed upload. In a real deployment one site administrator ingests
 * both the payroll claim and the turnstile log it is tested against; the
 * administrator's name is on both rows, but the turnstile produced the log.
 * Treating uploader as source would call every honest worker-day
 * self-certified, and did (retrodetect: 117 false positives from this one
 * rule).
 *
 * So a row is the submitter's own attestation when EITHER
 *   (a) its KIND is attestation-class — the kind's prior independence in
 *       `INDEPENDENCE_BY_KIND` is below `ATTESTATION_INDEPENDENCE_CEILING`
 *       (photograph, video, document, other, and any kind the table does not
 *       know). Content of that kind is composed by whoever submits it; a
 *       score the submitter declared for it cannot lift it out, because a
 *       declaration of independence made by the claimant about their own
 *       upload is itself self-certification; OR
 *   (b) the ROW ITSELF carries an assessed independence below the ceiling
 *       (0 < independenceScore < ceiling). An inlet that scores a source as
 *       not independent of the claimant population — the site-progress
 *       assessment does this when observer and claimant share an employer —
 *       is believed; that is a statement against the row, never for it. A
 *       score of 0 is "not scored" (the POST /evidence default), not an
 *       assessment.
 * Otherwise the row is a record produced upstream of its submitter and does
 * not count against independence whoever uploaded it.
 *
 * Known limit, stated rather than hidden: a hand-entered row of a record kind
 * (a "biometric_log" typed into POST /evidence) is taken at its label.
 * Closing that needs the inlet to bind the row to a registered source, which
 * is provenance work, not a separation-rule question.
 */
export function ownAttestation(
  ev: Pick<EvidenceLike, "kind" | "independenceScore">,
): { own: boolean; reason: string } {
  const prior = INDEPENDENCE_BY_KIND[ev.kind] ?? 0.2;
  const assessed =
    Number.isFinite(ev.independenceScore) && ev.independenceScore > 0
      ? ev.independenceScore
      : null;
  if (prior < ATTESTATION_INDEPENDENCE_CEILING) {
    return {
      own: true,
      reason:
        `a ${ev.kind} is composed by whoever submits it (kind independence ` +
        `${prior.toFixed(2)} < ${ATTESTATION_INDEPENDENCE_CEILING})`,
    };
  }
  if (assessed !== null && assessed < ATTESTATION_INDEPENDENCE_CEILING) {
    return {
      own: true,
      reason:
        `the row itself was scored ${assessed.toFixed(2)} for independence ` +
        `(< ${ATTESTATION_INDEPENDENCE_CEILING}): the inlet that wrote it said the source is ` +
        "not independent of the claimant population",
    };
  }
  return {
    own: false,
    reason:
      `a ${ev.kind} is a record produced upstream of its submitter (kind independence ` +
      `${prior.toFixed(2)}${assessed !== null ? `, scored ${assessed.toFixed(2)} on the row` : ""})`,
  };
}

/** Which side of the claim the submitter of a row stands on, if any. */
export type SelfSubmission = "claimant" | "author";

/**
 * Was this row submitted by someone who stands on the claim side — the named
 * claimant (user claims only; an entity cannot press upload) or the person who
 * authored the assertion, whoever it was filed for?
 */
export function selfSubmitted(
  ev: Pick<EvidenceLike, "submittedBy">,
  assertion: Pick<AssertionLike, "claimantId" | "claimantKind" | "createdBy">,
): SelfSubmission | null {
  if (assertion.claimantKind === "user" && ev.submittedBy === assertion.claimantId) {
    return "claimant";
  }
  if (assertion.createdBy && ev.submittedBy === assertion.createdBy) return "author";
  return null;
}

const ROLE_LABEL: Record<SelfSubmission, string> = {
  claimant: "the claimant",
  author: "the author of the assertion",
};

/** A short, reviewer-facing note on how a row arrived, from its provenance. */
export function provenanceNote(provenance: unknown): string | null {
  if (!provenance || typeof provenance !== "object") return null;
  const p = provenance as Record<string, unknown>;
  if (typeof p["via"] === "string") {
    return p["via"] === "ingestion_run" && typeof p["runId"] === "string"
      ? `via ingestion run ${p["runId"]}`
      : `via ${p["via"]}`;
  }
  if (typeof p["stream"] === "string") return `from stream ${p["stream"]}`;
  if (typeof p["observedBy"] === "string") return `observed by ${p["observedBy"]}`;
  return null;
}
 ...
 *
 * The identity rule is the sharp one: a row the claimant (or the author of
 * the assertion) both SUBMITTED and COMPOSED scores ZERO however good it
 * looks. That is the Vol III §4 design rule expressed as a number rather
 * than as a veto, so a reconciliation built only from such rows comes out
 * with no confidence rather than with false confidence. A record the same
 * person merely conveyed — see `ownAttestation` — keeps its base score:
 * the turnstile, the bank and the surveyor are not the uploader.
 */
export function effectiveIndependence(
  ...
  const role = selfSubmitted(evidence, assertion);
  if (role) {
    const own = ownAttestation(evidence);
    if (own.own) {
      return {
        score: 0,
        reason:
          `submitted by ${ROLE_LABEL[role]} and it is their own attestation (${own.reason}) — ` +
          "not independent testimony",
      };
    }
    return {
      score: base,
      reason:
        `uploaded by ${ROLE_LABEL[role]}, but ${own.reason}; independence ${base.toFixed(2)} stands`,
    };
  }
  return { score: base, reason: `independence ${base.toFixed(2)} for kind "${evidence.kind}"` };
}

/** One row's verdict under the separation rule, kept so a reviewer can see it. */
export interface SeparationRow {
  id: string;
  kind: string;
  submittedBy: string;
  /** null when the submitter stands on neither side of the claim */
  role: SelfSubmission | null;
  /** true when the row counts against independence: self-submitted AND own attestation */
  counted: boolean;
  reason: string;
}
...
export interface SelfCertification {
  ...
  /** the finding, written for the signal and the ledger; null when not self-certified */
  reason: string | null;
  rows: SeparationRow[];
}

/**
 * THE SEPARATION RULE, as one definition for every route that needs it.
 *
 * A reconciliation is self-certified when EVERY evidence row it used counts
 * against independence — each was submitted by the claimant or by the author
 * of the assertion AND is that person's own attestation (`ownAttestation`).
 * One record from an instrument or a third party among the rows, even one
 * the same person uploaded, means the claim was tested against something
 * outside itself and the rule does not bite; the weighting in
 * `effectiveIndependence` then handles the rest row by row.
 *
 * Every row's verdict is returned, counted or not, with the reason, so the
 * signal and the ledger can show a reviewer why a turnstile log uploaded by
 * the claim's author was not held against them and why their photograph was.
 */
export function selfCertification(
  assertion: Pick<AssertionLike, "claimantId" | "claimantKind" | "createdBy">,
  used: Array<Pick<EvidenceLike, "id" | "kind" | "independenceScore" | "submittedBy" | "provenance">>,
): SelfCertification {
  const rows: SeparationRow[] = used.map((ev) => {
    const role = selfSubmitted(ev, assertion);
    const own = ownAttestation(ev);
    const arrived = provenanceNote(ev.provenance);
    const reason = role
      ? `submitted by ${ROLE_LABEL[role]}${arrived ? ` (${arrived})` : ""}; ${own.reason}` +
        (own.own ? " — counted" : " — not counted")
      : `submitted by ${ev.submittedBy}, who stands on neither side of the claim — not counted`;
    return { id: ev.id, kind: ev.kind, submittedBy: ev.submittedBy, role, counted: role !== null && own.own, reason };
  });
  if (rows.length === 0 || !rows.every((r) => r.counted)) {
    return { selfCertified: false, reason: null, rows };
  }
  const roles = new Set(rows.map((r) => r.role));
  const who =
    roles.has("claimant") && roles.has("author")
      ? "the claimant and the author of the assertion"
      : roles.has("claimant")
        ? "the claimant"
        : "the author of the assertion";
  return {
    selfCertified: true,
    reason:
      `every evidence row used (${rows.map((r) => r.id).join(", ")}) was submitted by ${who} ` +
      "and is their own attestation, not a record from an instrument or a third party: " +
      rows.map((r) => `${r.id} — ${r.reason}`).join("; ") +
      ". A claim tested only against material produced by the person making it has not been " +
      "tested (Vol III §4).",
    rows,
  };
}
